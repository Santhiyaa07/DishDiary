...
const Recipes = () => {
  ...
  const getRecipes = () => {
    fetch(`${process.env.REACT_APP_API_URL}/auth/recipe`, {
      method: "GET",
      headers: {
    ...
    try {
      ...
      if (window.confirm("Are you sure you want to delete this recipe?")) {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/auth/recipe/${recipeId}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleAddToFavorites = async (recipeId) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/auth/likedRecipes/${recipeId}`,
        {
          method: "POST",
        }
      );
    ...
    try {
      if (e.target.value) {
        let Searchedrecipes = await fetch(
          `${process.env.REACT_APP_API_URL}/auth/searchRecipes/${e.target.value}`,
          {
            method: "GET",
            headers: {
